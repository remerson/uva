...
const unsigned MAX_N = 1024 * 1024;
...
/* This funt merges two sorted arrays and returns inversion count in
   the arrays.*/
inline long long merge(int arr[], int temp[], int left, int mid, int right)
{
  int i, j, k;
  long long counter = 0;
...
/* An auxiliary recursive function that sorts the input array and
  returns the number of inversions in the array. */
inline long long _mergeSort(int arr[], int temp[], int left, int right)
{
  int mid;
  long long counter = 0;
  if (right > left)
...
int main()
{
  ...
  while(inp.read_int(n))
  {
    ...
    DEBUG(dump());
    const long long counter = mergeSort(values, n);
    DEBUG(dump());
    printf("%lld\n", counter);
  }
