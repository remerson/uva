...
// #endif
  ...
  for(int s = 0; s < t; ++s)
  {
    ...
    for(int i = 0; i < n; ++i)
    {
      input.read_signed(scores[i]);
      DEBUG(printf("scores[%d] = %d\n", i, scores[i]));
    }

    int global_max = -99999999;
    int highest = scores[n-1];

    DEBUG(printf("start %d highest = %d\n", n-1, highest));

    for(int i = n-2; i >= 0; --i)
    { 
      DEBUG(printf("i = %d highest = %d max = %d\n", i, highest, global_max));

      const int value = scores[i] - highest;
